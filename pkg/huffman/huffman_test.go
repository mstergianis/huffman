...
func TestNewNodeFromBytes(t *testing.T) {
	...
	t.Run("single node tree", func(t *testing.T) {
		...
		expected := &Node{freqPair: &freqPair{b: 'r'}}
		Equal(t, expected, n, "expected: %08b actual: %08b", expected.freqPair.b, n.freqPair.b)
	})
	...
	t.Run("multi node tree", func(t *testing.T) {
		...
		expected := &Node{
			left: &Node{
				freqPair: &freqPair{b: 'l'},
			},
			right: &Node{
				freqPair: &freqPair{b: 'r'},
			},
		}
		Equal(t, expected, n)
	})
